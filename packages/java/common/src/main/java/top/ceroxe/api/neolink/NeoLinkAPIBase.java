 ...
 * socket、线程和活动转发连接。</p>
 */
abstract class NeoLinkAPIBase implements AutoCloseable {
    ...
    private volatile BiConsumer<String, Throwable> debugSink;
     ...
     * @param cfg 隧道配置
     */
    NeoLinkAPIBase(NeoLinkCfg cfg, BiConsumer<String, Throwable> defaultDebugSink) {
        this.cfg = Objects.requireNonNull(cfg, "cfg");
        this.debugSink = Objects.requireNonNull(defaultDebugSink, "defaultDebugSink");
    }

    protected abstract NeoLinkAPI self();

    protected abstract String platformUpdateClientType();

    protected abstract ExecutorService createWorkerExecutor();

    protected void requireStartThreadAllowed() {
        // Desktop can block the current thread by design; Android overrides this guard.
    }
    ...
    private String updateClientType() {
        return platformUpdateClientType();
    }
    ...
    private static InetSocketAddress parseRemoteAddress(String remoteAddress) {
        return NeoLinkProtocolSupport.parseRemoteAddress(remoteAddress, UNKNOWN_HOST);
    }
    ...
    private static String maskClientInfo(String clientInfo) {
        return NeoLinkProtocolSupport.maskClientInfo(clientInfo);
    }
     ...
     */
    public NeoLinkAPI setOnConnect(NeoLinkAPI.ConnectionEventHandler onConnect) {
     ...
     * @return 当前隧道实例，便于链式调用
     */
    public NeoLinkAPI setOnDisconnect(NeoLinkAPI.ConnectionEventHandler onDisconnect) {
     ...
     * 返回当前 PPv2 透传开关。
     *
     * <p>隧道运行时读取运行期配置快照；隧道未运行时读取构造 {@link NeoLinkAPI} 时持有的
     * {@link NeoLinkCfg}。这样 getter 与实际会被新 TCP 连接使用的配置保持一致。</p>
     *
     * @return {@code true} 表示之后新建的 TCP 连接会向本地下游透传 Proxy Protocol v2 头
     */
    public boolean isPPV2Enabled() {
        NeoLinkCfg activeCfg = runtimeCfg;
        if (running.get() && activeCfg != null) {
            return activeCfg.isPPV2Enabled();
        }
        return cfg.isPPV2Enabled();
    }

    /**
     * 运行期切换 PPv2 透传开关。
     *
     * <p>PPv2 是本地转发器行为，不需要通知 NeoProxyServer。这里同时写入原始配置和
     * 运行期配置：前者保证下一次 {@link #start()} 仍沿用最后一次 API setter 写入的值，
     * 后者保证当前运行中的隧道从下一条新建 TCP 连接开始生效。已建立的 TCP 连接在
     * {@code TCPTransformer} 构造时已经取得快照，不会被中途改变。</p>
     *
     * @param ppv2Enabled 是否向本地下游透传 Proxy Protocol v2 头
     * @return 当前隧道对象，便于链式调用
     */
    public NeoLinkAPI setPPV2Enabled(boolean ppv2Enabled) {
        synchronized (lifecycleLock) {
            cfg.setPPV2Enabled(ppv2Enabled);
            NeoLinkCfg activeCfg = runtimeCfg;
            if (activeCfg != null) {
                activeCfg.setPPV2Enabled(ppv2Enabled);
            }
        }
        debug("Runtime PPv2 passthrough switched. ppv2Enabled=" + ppv2Enabled);
        return self();
    }
     ...
     * @return 当前隧道对象，便于链式调用
     */
    public NeoLinkAPI setPPV2Enabled() {
    ...
    private void handleServerCommand(String command) throws IOException, NoSuchKeyException, NoMoreNetworkFlowException {
        ...
        switch (parts[0]) {
            case "sendSocketTCP" -> {
                if (isDispatchEnabled(NeoLinkAPI.TransportProtocol.TCP) && parts.length >= 3) {
            ...
            case "sendSocketUDP" -> {
                if (isDispatchEnabled(NeoLinkAPI.TransportProtocol.UDP) && parts.length >= 3) {
    ...
    private void emitConnectionEvent(NeoLinkAPI.ConnectionEventHandler handler, NeoLinkAPI.TransportProtocol protocol, String remoteAddress) {
        NeoLinkCfg activeCfg = runtimeCfg;
    ...
    private boolean isDispatchEnabled(NeoLinkAPI.TransportProtocol protocol) {
        settlePendingProtocolSwitchIfExpired();
        NeoLinkCfg activeCfg = runtimeCfg;
        boolean configuredEnabled = protocol == NeoLinkAPI.TransportProtocol.TCP
    ...
    private record ProtocolFlags(boolean tcpEnabled, boolean udpEnabled) {
        ...
        boolean enabled(NeoLinkAPI.TransportProtocol protocol) {
            return protocol == NeoLinkAPI.TransportProtocol.TCP ? tcpEnabled : udpEnabled;
        }
    ...
    private record ProtocolSwitch(
        ...
        boolean accepts(NeoLinkAPI.TransportProtocol protocol) {
