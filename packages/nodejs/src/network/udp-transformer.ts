...
export function serializeDatagramPacket(message: Buffer | Uint8Array, rinfo: { address: string; port: number }): Buffer {
  ...
  let offset = 0;
  out.writeUInt32BE(MAGIC, offset); offset += 4;
  out.writeInt32BE(payload.length, offset); offset += 4;
  out.writeInt32BE(ipBytes.length, offset); offset += 4;
  ipBytes.copy(out, offset); offset += ipBytes.length;
  out.writeUInt16BE(rinfo.port, offset); offset += 2;
...
export function deserializeToDatagramPacket(serializedData: Buffer | Uint8Array, debug: DebugSink = () => undefined): DatagramPacket | null {
  ...
  const dataLen = data.readInt32BE(offset); offset += 4;
  const ipLen = data.readInt32BE(offset); offset += 4;
  ...
  const ipBytes = data.subarray(offset, offset + ipLen); offset += ipLen;
...
function createDatagramQueue(socket: dgram.Socket): DatagramQueue {
  ...
  const waiters: Array<{ resolve: (value: { message: Buffer; rinfo: dgram.RemoteInfo } | null) => void; reject: (error: Error) => void }> = [];
  let closed = false;
