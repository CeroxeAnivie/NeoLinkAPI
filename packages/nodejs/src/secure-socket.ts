...
import {closeQuietly, connectTcp, SocketReader, writeAll} from './network/socket-utils.js';
...
export class SecureSocket {
    ...
    static getMaxAllowedPacketSize(): number {
    ...
    static getDefaultConnectTimeoutMillis(): number {
    ...
    getRemoteSocketAddress(): Endpoint {
        return {host: this.socket?.remoteAddress ?? 'unknown', port: this.socket?.remotePort ?? 0};
    }

    getLocalSocketAddress(): Endpoint {
        return {host: this.socket?.localAddress ?? 'unknown', port: this.socket?.localPort ?? 0};
    }
...
export class SecureServerSocket {
    ...
    private readonly waiters: Array<{
        resolve: (socket: SecureSocket) => void;
    ...
    addIgnoreIP(ip: string): void {
        this.ignoreIPs.add(ip);
    }

    removeIgnoreIP(ip: string): boolean {
        return this.ignoreIPs.delete(ip);
    }

    getIgnoreIPs(): string[] {
        return Array.from(this.ignoreIPs);
    }
