...
桌面 JVM 产物坐标是 `top.ceroxe.api:neolinkapi-desktop:7.2.4`。共享模型与 NKM 节点工具坐标是 `top.ceroxe.api:neolinkapi-shared:7.2.4`，其中包含 `NodeFetcher`、`NeoNode` 和 `NeoLinkCfg`。Android 产物使用独立坐标 `top.ceroxe.api:neolinkapi-android:7.2.4`，不要在 Android 项目中依赖桌面 JVM 产物。
