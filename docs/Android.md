...
Android 产物坐标是 `top.ceroxe.api:neolinkapi-android:7.2.2`。共享模型与 NKM 节点工具坐标是 `top.ceroxe.api:neolinkapi-shared:7.2.2`，其中包含 `NodeFetcher`、`NeoNode` 和 `NeoLinkCfg`。不要在 Android 项目中依赖桌面 JVM 产物 `top.ceroxe.api:neolinkapi-desktop`。
...
### 进入 `android/neolinkapi-android` 后执行
